...
public class UserDetails extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if(!GMS.getGMS().logedIn()){    // verifica se o user está logado, se não tiver envia o erro 401, e no lado cliente vai ser pedido o login
            ...
            if(method.equals("POST") && target.equals("user")){
                ...
                String name = (String) postContent.get("name");
                try {
                    user = GMS.getGMS().getUserDetails(name);   //  if(GMS.getGMS().login(name, password);
                ...
                if(user != null)data = mapper.writeValueAsString(user);
                else data="";
