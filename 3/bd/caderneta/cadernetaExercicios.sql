...
-- 1
SELECT *
	FROM Jogador WHERE Equipa IN (SELECT id FROM Equipa WHERE Designacao='Sporting Clube de Braga' AND Deginacao='Rio Ave Futebol Clube')
		AND Posicao IN (SELECT id FROM Posicao WHERE Designacao ='Defesa')
        ANd c.jogador =j.Nr;
   
  -- 2     
SELECT c.Nr as Numero, j.Nome as Nome, p.designacai as Posicao
...
-- 4
-- DELIMITER ||
-- CREATE PROCEDURE parametros (IN __ Type, OUT __ Type, OUT ..., OUT ...)
-- BEGIN
-- (...)
-- END //
-- DELIMITER ||
...
CREATE PROCEDURE obter_cromos_equipa (IN Clube VARCHAR(45))
	BEGIN
		SELECT c.Nr as Número, c.PagCaderneta as Pagina, j.Nome, as Nome, c.Adquirido
			FROM Cromo c, Jogador j, Equipa e
				WHERE c.jogador = j.Nr
					AND j.Equipa = e.id AND e.Designacao = Clube
					ORDER BY c.PagCaderneta ASC, c.Nr ASC;
	END //
DELIMITER //

CALL obter_cromos_equipa ('Sporting Clube de Braga');

-- 5 
DELIMITER //
CREATE PROCEDURE obter_todos_os_cromos()
	BEGIN
		SELECT c.Nr, tc.Descricao, j.Nome, c.Adquirido, e.Descricao
...
-- 6 
DELIMITER //
CREATE FUNCTION repetido(numero INT) RETURNS VARCHAR(15)
	DETERMINISTIC
		BEGIN
...
-- 7 não dá para fazer com uma função, pq não sei retornar mais do que um valor, logo resolvo com o procedure

-- 8 

-- DELIMITER //
-- CREATE TRIGGER ___ [AFTER|BEFORE][INSERT |UPDATE| DELETE] ON TABLE
-- FOR EACH ROW
-- BEGIN
-- ...

DELIMITER //
CREATE TRIGGER mudar_valor_adquirido AFTER UPDATE ON Cromo
	FOR EACH ROW
	BEGIN
