...
int designStrit(int h, int index){
...
int generatePointsGrid(){
	...
	for(int i = -altura; i <= altura; i++){
		index = designStrit(i, tmp);
		tmp = index;

	}

	return index;	//	número de pontos
...
// 	Build the vertex arrays
	...
	vertexB = (float*) malloc(arraySize*100*sizeof(float));

	generatePointsGrid();

	for(int i = 0; i < arraySize; ){
		printf("vertexB[%d]=%.2f | ", i, vertexB[i++]);
		printf("vertexB[%d]=%.2f | ", i, vertexB[i++]);
		printf("vertexB[%d]=%.2f\n", i, vertexB[i++]);
	}
	
	/*	VBO - colocar os indices - mandar para a placa  */
